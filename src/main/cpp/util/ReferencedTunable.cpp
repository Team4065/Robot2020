...
std::map<const char*, std::shared_ptr<double>> ReferencedTunable::registered_tunables_;
unsigned char ReferencedTunable::ref_count_ = 0;

void ReferencedTunable::Register(const char* name, std::shared_ptr<double> data)
{
    registered_tunables_.insert(rt_pair(name, data));
    ref_count_++;

    frc::SmartDashboard::PutNumber(name, *data.get());
}
